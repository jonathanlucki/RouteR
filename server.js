...
const formatTimeMatrix = matrix => {
  var formattedMatrix = [];
  for (var rowNum = 0; rowNum < matrix.rows.length; rowNum++) {
    var rowArray = [];
    for (
      var colNum = 0;
      colNum < matrix.rows[rowNum].elements.length;
      colNum++
    ) {
...
const getTravelTimeMatrix = (addressArray, callback) => {
...
const getAddressArray = locations => {
  var array = [];
  //add start to array
  if (locations.start !== "") {
...
const getPathDuration = (path, pathLength, timeMatrix) => {
  var time = 0;
...
const calculateRoute = (locations, callback) => {
  ...
  getTravelTimeMatrix(addressArray, function(resultMatrix) {
    ...
    if (resultMatrix !== "error") {
      ...
      var minDuration = getPathDuration(
        pathsArray[0],
        pathsArray[0].length,
        resultMatrix
      );
      for (var k = 1; k < pathsArrayLength; k++) {
        if (
          getPathDuration(pathsArray[k], pathsArray[k].length, resultMatrix) <
          minDuration
        ) {
